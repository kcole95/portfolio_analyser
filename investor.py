...
# @timer
def add_tradeables(tradeables=[], portfolio=None, historical_prices_tradeables=None):
	...
	if True:
		for i in tradeables:
			...
			# Already have price data for some etfs:
			try:
				prices = historical_prices_tradeables['ETF'][i]
				quotetype = 'ETF'
			except:

				try:
					prices = yf.Ticker(i).history(period='max')['Close']
				except:
					prices = 0

				# Obtain the quotype of the instrument - ETF, CRYPTOCURRENCY, EQUITY etc.
				try:
					quotetype = yf.Ticker(i).info['quoteType']
				except:
					quotetype = 'MISC'
				
			# Save the prices for later use.
			historical_prices_tradeables[quotetype][i] = prices

			if i == 'EQQQ.L':
				portfolio[i] = {'name': 'PowerShares EQQQ Nasdaq-100 UCITS ETF',
				'type': 'ETF',
				'sector': 'n/a',
				'index': '^IXIC',
				'w_return': logarithmic_returns(historical_prices_tradeables['ETF'][i].iloc[-2520:]).mean() * 250,
				'w_beta': 1,
				'w_std': (stock_variance(logarithmic_returns(historical_prices_tradeables['ETF'][i].iloc[-2520:])) * 250) ** 0.5,
				'w_capm': 'n/a',
				'w_sharpe': 'n/a'}

			else:
				# Add the tradeable to the portfolio. Some do not have a 'longName' in the yfinance module, but they do have a 'shortName'.
				

				try:
					portfolio[i] = {'name': yf.Ticker(i).info['longName'],
					'type': quotetype,
					'sector': 'n/a',
					'index': 'n/a',
					'w_return': logarithmic_returns(prices.apply(gbx_to_gbp).iloc[-2520:]).mean() * 250,
					'w_beta': 'n/a',
					'w_std': (stock_variance(logarithmic_returns(prices.apply(gbx_to_gbp).iloc[-2520:])) * 250) ** 0.5,
					'w_capm': 'n/a',
					'w_sharpe': 'n/a'}
				except:
					portfolio[i] = {'name': yf.Ticker(i).info['shortName'],
					'type': quotetype,
					'sector': 'n/a',
					'index': 'n/a',
					'w_return': logarithmic_returns(prices.apply(gbx_to_gbp).iloc[-2520:]).mean() * 250,
				...
				# Use re to check what name is in the ETF to find the index
				phrases = {'FTSE':'^FTSE', 'Nasdaq':'^IXIC', '500':'^GSPC', 'Gold':'^FTSE', 'coin':'^CMC200'}
				for key in phrases:
					if key in portfolio[i]['name'].split():
						portfolio[i]['index'] = phrases[key]
						portfolio[i]['w_beta'] = 1
					elif portfolio[i]['type'] == 'CRYPTOCURRENCY':
						portfolio[i]['index'] = phrases[key]

			print('Successfully added ' + i + ' to the revised_portfolio.')
